...
int controller_encontrarClienteConMasVentas (LinkedList*pArrayListVentas,LinkedList*pArrayListClientes)
{
	...
	int indice;
	int idClienteAux;
	indice =ventas_encontrarMayorOMenor(pArrayListVentas,pArrayListClientes,0);
...
int controller_encontrarClienteConMenosVentas (LinkedList*pArrayListVentas,LinkedList*pArrayListClientes)
{
	...
	int indice;
	int idClienteAux;
	indice =ventas_encontrarMayorOMenor(pArrayListVentas,pArrayListClientes,1);
	Ventas * ventaAux = ll_get(pArrayListVentas, indice);
	ventas_getIdCliente(ventaAux, &idClienteAux);
	ll_mapAndBreak(pArrayListClientes,cliente_printOne,idClienteAux);
	return retorno;
...
int ventas_encontrarMayorOMenor(LinkedList* pArrayListVentas,LinkedList* pArrayListClientes,int mayor){
	...
	for (int i = 0; i<ll_len(pArrayListVentas); i++)
	{
		ventasAux = ll_get(pArrayListVentas, i);
		ventas_getIdCliente(ventasAux, &idClienteAux);
		ll_reduceIntbyID(pArrayListVentas, cantidadAfichesporId, &cantidadAfiches, idClienteAux);
		if (mayor == 0)
		{
